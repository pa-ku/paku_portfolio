      ...
      fetch("https://api.emailjs.com/api/v1.0/email/send", {
        ...
        .then((response) => {
          if (response.ok) {
            ...
            console.error(
              "Hubo un error al enviar el correo" + response.status,
            );
          }
        })
        .catch((error) => {
        ...
        >
          <span>
            ...
            <p className="text-sm dark:text-slate-300 text-slate-700 ">
              {spanish
                ? "Si te gusta mi trabajo o experiencia, no dudes en contactarme. Me interesan los proyectos ambiciosos. Pero si tenes otras peticiones o preguntas ¡no dudes en consultarme!"
          ...
          <input
            ...
            className="dark:bg-background-600 shadow-lg px-4 py-2 rounded-lg"
            type="text"
            placeholder="Email"
          />
          <textarea
            onChange={(e) => setMessage(e.target.value)}
            className="min-h-20 max-h-40 shadow-lg dark:bg-background-600 px-4 py-2 rounded-lg"
            placeholder="Hola pablo!"
            name=""
            id=""
          ></textarea>
          <button
            onClick={sendForm}
            className="bg-primary py-2 text-white rounded-lg "
          >
            {spanish ? "Enviar" : "Send"}
          </button>
