...
export default function Card({ proyect }) {
  ...
  return (
    <>
      {proyect.docs && (
        <DocsModal
          data={spanish ? proyect.docs.es : proyect.docs.eng}
        ></DocsModal>
      )}
          ...
          <div className="w-full flex justify-between">
            <span>
              <span className="flex gap-3 items-center">
                <h2 className="filter-gradient-text">
                  <b>{proyect.title}</b>
                </h2>
              </span>
...
function ProyectButton({ onClick, className, children }) {
  return (
    <button
      onClick={onClick}
      target="blank"
      className={`duration-300 dark:bg-background-600  hover:text-white   border-2 flex items-center justify-center gap-1 rounded-lg py-1 px-2 border-current;  ${className}   `}
    >
      {children}
    </button>
  );
