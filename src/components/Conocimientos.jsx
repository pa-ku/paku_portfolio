...
export default function Conocimientos() {
    ...
    return (
        <aside className='flex flex-col gap-5  w.full items-center justify-center'>
            ...
            <div className="flex flex-wrap gap-7 items-center justify-center">
                {MY_STACK.map(({ icon, text, description }, index) => (
                    <div key={index} className="relative group flex items-center justify-center size-16  flex-col">
                        ...
                        {description && <ol className="text-sm flex-col gap-2 list-disc -bottom-20 duration-300 flex border-background-200 border-2 rounded-lg -left-48  w-64 bg-white/80 dark:bg-background-900/60 backdrop-blur-sm z-50 p-3  group-hover:opacity-100 absolute opacity-0 pointer-events-none">{description.map(text => (
